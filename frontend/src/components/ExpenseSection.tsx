...
const ExpenseSectionComponent = ({
  ...
  const handleSubmit = async (data: Omit<Expense, 'id'>) => {
    if (editingExpense) {
      ...
      // Mudança de tipo: sempre só o registro aberto (Q1-A)
      const typeChanged = data.type !== editingExpense.type;
      const success = await onUpdate(
        editingExpense.id,
        updates,
        typeChanged ? false : applyToAllMonths
      );
        ...
        <DialogContent className="rounded-lg max-w-md">
            ...
            <TabsList className="grid w-full grid-cols-3 rounded-md bg-muted p-1">
              ...
              >
                ...
                {/* Um único form: troca de aba só muda `type`, preservando campos comuns (DEV-104) */}
                <ExpenseForm
                  key={editingExpense?.id ?? (addDraft ? 'draft' : 'new')}
                  type={activeTab}
                  categories={categories}
                  ...
                  accounts={accounts}
                  wishConquerPlannedValue={addDraft ? wishConquerPlannedValue : undefined}
                  initialData={
                    editingExpense
                      ? editingExpense
                      : addDraft
                        ? buildDraftExpense(addDraft.type ?? 'variable', addDraft)
                        : undefined
                  }
                  categoryManagerSlot={
                    <Popover

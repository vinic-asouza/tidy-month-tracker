...
import { MonthData, FinanceSettings } from '@/types/finance';
...
export const Statistics = ({ yearData, currentYear, monthData, settings }: StatisticsProps) => {
  const yearStats = useMemo(() => {
    const totals = yearData.reduce(
      (acc, month) => ({
        income: acc.income + month.incomes.filter(i => i.received).reduce((s, i) => s + i.value, 0),
        expenses: acc.expenses + month.expenses.filter(e => e.paid).reduce((s, e) => s + e.value, 0),
    ...
    const monthlyData = yearData.map((month, index) => ({
      ...
      Gastos: month.expenses.filter(e => e.paid).reduce((s, e) => s + e.value, 0),
